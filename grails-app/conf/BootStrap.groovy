...
//					println("Producer: " + prod)
		...
		def portalAdminRole  = MpoRole.findByAuthority("ROLE_PORTAL_ADMIN")?: new MpoRole(authority:"ROLE_PORTAL_ADMIN").save()
		def portalUserRole  = MpoRole.findByAuthority("ROLE_PORTAL_USER")?: new MpoRole(authority:"ROLE_PORTAL_USER").save()
		...
		if(!users){
			...
			def portalAdmin = User.findByUsername('mpoadmin') ?: new User(
					username: 'mpoadmin',
					password: 'mpoadmin',
					fullName : 'mpoadmin',
			...
			def portalUser = User.findByUsername('portaluser') ?: new User(
				username: 'portaluser',
				password: 'portaluser',
				fullName : 'siteUser',
				email : 'kumarvayyala@outlook.com',
			...
			def publisherAdmin = User.findByUsername('publisheradmin') ?: new User(
					...
					password: 'publisheradmin',  //pw encoded by security plugin
					fullName : 'publisheradmin',
					email : 'kumarvayyala@outlook.com',
					mobile:'9849889255',
					gender :'male',
					dob : new Date(),
			...
			def endUser = User.findByUsername('enduser') ?: new User(
					username: 'enduser',
					password: 'enduser',
					fullName : 'enduser',
					email : 'kumarvayyala@outlook.com',
			...
			if (!portalAdmin.authorities.contains(portalAdminRole)) {
				MpoUserRole.create portalAdmin, portalAdminRole
			}
			
			if (!portalUser.authorities.contains(portalUserRole)) {
				MpoUserRole.create portalUser, portalUserRole
			}
			if (!publisherAdmin.authorities.contains(publisherAdminRole)) {
				MpoUserRole.create publisherAdmin, publisherAdminRole
			}

			if (!publisherUser.authorities.contains(publisheruserRole)) {
				MpoUserRole.create publisherUser, publisheruserRole
			}

			if (!endUser.authorities.contains(enduserRole)) {
				MpoUserRole.create endUser, enduserRole
			}
		}
