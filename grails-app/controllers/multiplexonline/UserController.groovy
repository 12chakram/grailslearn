 ...
 */
 @Secured(['ROLE_PORTAL_ADMIN','ROLE_PUBLISHER_ADMIN'])
class UserController extends grails.plugin.springsecurity.ui.UserController {
	...
	def showInvite(){
		...
		invitedUsers = InvitationCode.findAll(max:9,sort:"dateCreated",order:'desc') { username :currentUser.username }
		
		def role = springSecurityService.getPrincipal().getAuthorities()
		
		String currentUserRole = role.getAt(0).toString()
		
		if(currentUserRole.equals('ROLE_PORTAL_ADMIN')){
			roles =	MpoRole.findAll("from MpoRole as r where r.authority in (:authority)",
					[authority: ['ROLE_PUBLISHER_ADMIN', 'ROLE_PORTAL_USER']])
		}else if(currentUserRole.equals('ROLE_PUBLISHER_ADMIN')){
		 roles = MpoRole.findAll("from MpoRole as r where r.authority in (:authority)",
			 [authority: ['ROLE_PUBLISHER_USER']])
		}
		setRoleName();
	...
	def sendInvitation(){
		...
		try{
			...
			mailService.sendMail {
				...
				subject senderName+' Invite you as a '+invitationCodeInstance.invitee+' on Multiplex Online'
				//html body.toString()
	...
	def allinvitees(){
		...
		def allInvitedUsers = InvitationCode.findAll(sort:"dateCreated",order:'desc') { username :currentUser.username }
		[allInvitedUsers : allInvitedUsers]
	}
	
	
	private def setRoleName(){
		roles.each{
			if(it.authority.equals('ROLE_PORTAL_USER')){
				it.roleName = 'Portal User'
			}else if(it.authority.equals('ROLE_PUBLISHER_ADMIN')){
			  it.roleName = 'Publisher Admin'
			}
			else if(it.authority.equals('ROLE_PUBLISHER_USER')){
				it.roleName = 'Publisher USer'
			  }
		}
	}
