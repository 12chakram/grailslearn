...
<div class="fieldcontain ${hasErrors(bean: videoObjectInstance, field: 'publisher', 'error')} required">
	...
	<g:select id="publisher" name="publisher.id" from="${org.schema.Organization.findAllByIsMediaPublisher(true)}" optionKey="id" required="" value="${videoObjectInstance?.publisher?.id}" class="many-to-one"/>
