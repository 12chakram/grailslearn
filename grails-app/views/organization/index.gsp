		...
		<div id="list-organization" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="sanitizedName" title="${message(code: 'organization.sanitizedName.label', default: 'Sanitized Name')}" />
					
						<g:sortableColumn property="alternateName" title="${message(code: 'organization.alternateName.label', default: 'Alternate Name')}" />
					
						<g:sortableColumn property="description" title="${message(code: 'organization.description.label', default: 'Description')}" />
					
						<g:sortableColumn property="image" title="${message(code: 'organization.image.label', default: 'Image')}" />
					
						<g:sortableColumn property="sameAs" title="${message(code: 'organization.sameAs.label', default: 'Same As')}" />
				...
				<g:each in="${organizationInstanceList}" status="i" var="organizationInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: organizationInstance, field: "sanitizedName")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "alternateName")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "description")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "image")}</td>
					
						<td>${fieldValue(bean: organizationInstance, field: "sameAs")}</td>
